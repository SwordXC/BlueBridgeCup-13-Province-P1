...
{
	LCD_DisplayStringLine(Line,Column0, "                    ");
}
/*******************************************************************************
...
* Return         : None
*******************************************************************************/
void LCD_DisplayStringLine(u8 Line,u16 Column, u8 *ptr)
{
	u32 i = 0;
...
/*******************************************************************************
*  函数名称：LCD_DispDecAt(uint8_t line,uint8_t x,int32_t number,uint8_t len)
*  功能说明：LCD显示一个的整形数字
*  参数说明：
             uint8_t line：行位置（0-9）
             uint8_t x   ：列位置（0-19）
...
*  使用示例：
********************************************************************************/
void LCD_DispDecAt(uint8_t line,uint16_t x,int32_t number,uint8_t len)
{
    char buf[30];
    int8_t p,flag=0;
    uint32_t temp;
    ...
    LCD_DisplayStringLine(line,x,buf);
}
/*******************************************************************************
*  函数名称：LCD_DispFolatAt(uint8_t line,uint8_t x,float number,uint8_t len)
*  功能说明：LCD显示一个的浮点数
*  参数说明：  uint8_t line：行位置（0-9）
             uint16_t x   ：列位置（0-19）
...
*  使用示例：
********************************************************************************/
void LCD_DispFolatAt(uint8_t line,uint16_t x,float number,uint8_t len)
{
    const float _fround[6]={0.49,0.05,0.005,0.0005,0.00005,0.000005};
    unsigned char i=0,d;
    uint8_t num1,num2,num3;//num1是小数位数，num2是整数位数，num3是负号
    char Str[20];
    char * str=Str;
    float scale = 1.0;
    num1=len;
    ...
    {
        *str=0;
        LCD_DisplayStringLine(line,x,Str);
        LCD_DisplayStringLine(line,x-(num1+num2+num3+1)*16,"   ");
        return;
    }
    *str++='.';
    while (len--)
